...
struct CombinedResidual {
  CombinedResidual(int size, const double* data, int threads)
      : size_{size}, data_{data}, threads_{threads} {}

  template <typename T>
  bool operator()(const T* const m, const T* const c, T* residual) const {
    const int gsize = size_ / threads_;

    //    using PrivVec = tbb::enumerable_thread_specific<std::vector<T>>;
    //    PrivVec priv_res;

    tbb::parallel_for(
        tbb::blocked_range<int>(0, size_, gsize), [&](const auto& blk) {
          //          auto& local = priv_res.local();

          for (int i = blk.begin(); i < blk.end(); ++i) {
            residual[i] = data_[2 * i + 1] - exp(m[0] * data_[2 * i] + c[0]);
            //            local.push_back(data_[2 * i + 1] - exp(m[0] * data_[2
            //            * i] + c[0]));
          }
        });

    //    int k = 0;
    //    for (const auto& each : priv_res) {
    //      for (const auto& r : each) {
    //        residual[k++] = r;
    //      }
    //    }
...
void BM_ExponentialResidual(benchmark::State& state) {
  for (auto _ : state) {
    double m = 0;
    double c = 0;
...
BENCHMARK(BM_ExponentialResidual)->Arg(1)->Arg(2)->Arg(4);

void BM_CombinedResidual(benchmark::State& state) {
  for (auto _ : state) {
    double m = 0;
    double c = 0;
    ...
    Problem problem;
    problem.AddResidualBlock(
        new AutoDiffCostFunction<CombinedResidual, ceres::DYNAMIC, 1, 1>(
            new CombinedResidual(kNumObservations, data, state.range(0)),
            kNumObservations),
        nullptr,
        &m,
        &c);
...
BENCHMARK(BM_CombinedResidual)->Arg(1)->Arg(2)->Arg(4);
