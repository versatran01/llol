...
/// GicpCostLinear =============================================================
bool GicpCostLinear::Compute(const double* px, double* pr, double* pJ) const {
  const State es(px);
  const SO3d eR = SO3d::exp(es.r0());
  ...
  tbb::parallel_for(
      tbb::blocked_range<int>(0, matches.size(), gsize_), [&](const auto& blk) {
        for (int i = blk.begin(); i < blk.end(); ++i) {
          ...
          const double s = (match.px_g.x + 0.5) / pgrid->cols();
          ...
          if (pJ != nullptr) {
            ...
            U *= w_icp;  // scale jacobian
            J.block<3, 3>(ri, Block::kR0 * 3) = U * Hat3(pt_p_hat);
            J.block<3, 3>(ri, Block::kP0 * 3) = U * (s - 1.0);
            J.block<3, 3>(ri, Block::kP1 * 3) = U * (-s);
          }
        }
      });
  ...
  // beta residual
  // r_beta = R0^T (v1 + g * dt - v0) - beta
  //        = R0^T ((ep1 - ep0) / dt + dv) - beta
  const Vector3d dv = g_p * dt;
  r_imu.segment<3>(3) = R0_t * (dep / dt + dv) - preint.beta;
  ...
  if (pJ != nullptr) {
    ...
    Eigen::Map<RowMatXd> J(pJ, NumResiduals(), NumParameters());
    // alpha jacobian
    J.block<3, 3>(offset, Block::kR0 * 3) = R0_t_mat * Hat3(dp);
    J.block<3, 3>(offset, Block::kP0 * 3) = -R0_t_mat;
    J.block<3, 3>(offset, Block::kP1 * 3) = R0_t_mat;

    // beta jacobian
    J.block<3, 3>(offset + 3, Block::kR0 * 3) = R0_t_mat * Hat3(dv);
    J.block<3, 3>(offset + 3, Block::kP0 * 3) = -R0_t_mat / dt;
    J.block<3, 3>(offset + 3, Block::kP1 * 3) = R0_t_mat / dt;

    // gamma jacobian
    J.block<3, 3>(offset + 6, Block::kR0 * 3) = R0_t_mat;
    J.block<3, 3>(offset + 6, Block::kP0 * 3).setZero();
    J.block<3, 3>(offset + 6, Block::kP1 * 3).setZero();

    J.block<9, 9>(offset, 0).applyOnTheLeft(U);
  }

  return true;
}

void GicpCostLinear::UpdateTraj(Trajectory& traj) const {
  const auto dt = traj.duration();
  const State es(error.data());
  const auto eR = SO3d::exp(es.r0());
  LOG(INFO) << error.transpose();

  // Only update first state, the rest will be done in repredict
  auto& st = traj.states.front();
  st.rot = eR * st.rot;
  st.pos = eR * st.pos + es.p0();
  st.vel = eR * st.vel + (es.p1() - es.p0()) / dt;
}
